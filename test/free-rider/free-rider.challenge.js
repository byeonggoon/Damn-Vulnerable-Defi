...
describe("[Challenge] Free Rider", function () {
  ...
  before(async function () {
    ...
    await this.uniswapRouter.addLiquidityETH(
      ...
      UNISWAP_INITIAL_TOKEN_RESERVE, // amountTokenDesired // 15000
      0, // amountTokenMin
      0, // amountETHMin
      deployer.address, // to
      (await ethers.provider.getBlock("latest")).timestamp * 2, // deadline
      { value: UNISWAP_INITIAL_WETH_RESERVE } // 9000
    );
    ...
    console.log(await this.nft.balanceOf(deployer.address));
    ...
    console.log("before swap ether", (await attacker.getBalance()).toString());
    console.log(
      "before swap token",
      (await this.token.balanceOf(attacker.address)).toString()
    );

    const ATTACKER_INIT_ETH = ethers.utils.parseEther("0.4");
    ...
    await this.uniswapRouter
      ...
      .swapExactETHForTokens(
        ...
        { value: ATTACKER_INIT_ETH }
      );

    console.log("after swap ether", (await attacker.getBalance()).toString());
    console.log(
      "after swap token",
      (await this.token.balanceOf(attacker.address)).toString()
    );
