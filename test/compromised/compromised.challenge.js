...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    function hexToText(hex) {
      var hex = hex.toString().replace(/\s+/g, "");
      var str = "";
    ...
    const zinToPrivateKey = (hex) => {
      var hex = hex.toString().replace(/\s+/g, "");
      var str = "";
      for (var i = 0; i < hex.length; i += 2)
        str += String.fromCharCode(parseInt(hex.substr(i, 2), 16));
      var decodedString = Buffer.from(str, "base64").toString();
      return decodedString;
    };

    function base64Decode(encodedString) {
      // var decodedString = atob(encodedString);
      var decodedString = Buffer.from(encodedString, "base64").toString();
      return decodedString;
    }

    // function base64Decode(encodedString) {

    const newpKey1 = zinToPrivateKey(leakedInformation[0]);
    console.log("newpKey1", newpKey1);

    const hexToText1111 = hexToText(leakedInformation[0]);
    console.log("hexToText1111", hexToText1111);
    const catchPkey = base64Decode(hexToText1111);
    console.log("catchPkey", catchPkey);

    const base64key1 = leakTobase64(leakedInformation[0]);
    const base64key2 = leakTobase64(leakedInformation[1]);
    console.log("base64key1", base64key1);
    console.log("base64key2", base64key2);
